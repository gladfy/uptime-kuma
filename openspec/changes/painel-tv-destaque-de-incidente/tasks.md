...
- [x] 8.5 O painel passa a usar o "Intervalo de atualização" da status page (decisão 6 revista): o endpoint publica `status_page.auto_refresh_interval` (padrão 60 s) e a leitura virou cadeia de `setTimeout`, com piso de 5 s e releitura do valor a cada ciclo. Medido no navegador: 8 s dá leituras de 8 em 8; trocando para 5 com o painel ligado, a cadência muda sozinha; 1 s é elevado ao piso de 5 s; 0, nulo e texto caem em 60 s. Dois testes novos no `test-status-page-tv-endpoint.js` (valor configurado e fallback).
